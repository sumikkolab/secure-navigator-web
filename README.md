...
SecureNavigator は、組織のセキュリティ対策状況を攻撃者の視点から分析し、想定攻撃に対する防御上の弱点を整理するオフライン評価ツールです。MITRE ATT&CK ベースの6つの攻撃シナリオに対して防御力を分析し、IPA 中小企業向けガイドライン・経産省サイバーセキュリティ経営ガイドライン・NIST CSF 2.0 を参考にした対応状況を整理します。着手候補を時間軸別にまとめ、説明・報告・改善検討に使えるレポートを生成します。すべての処理はローカルで完結し、外部にデータを送信しません。

## 想定利用者

- **情シス担当者** — セキュリティ対策状況を整理し、社内報告用の資料を作成する
- **セキュリティ担当者** — 防御上の弱点を特定し、着手候補を検討する
...
- **複数ガイドライン参考確認** — IPA は Free で利用可能。経産省ガイドラインと NIST CSF 2.0 は Pro で利用可能
- **対応候補の整理** — 「今すぐ」「短期」「中期」「長期」の4段階で着手候補を表示
- **レポート出力** — PDF、HTML、Markdown で診断結果を出力 [Pro]
- **高度な分析** — 成熟度詳細・原因と対策テーブル・ATT&CK テクニック詳細分析 [Pro]
- **AI 関連参考確認** — NIST AI RMF / 総務省 AI セキュリティガイドライン / UK NCSC Secure AI System Development 関連ガイダンス を参考にした確認結果を表示 [Pro]（参考確認であり、準拠・認証・適合判定を示すものではありません）
- **日本語 / 英語** — 診断・レポートともに日本語と英語に対応

## v1.2 更新内容

- 動作基盤を .NET 10 に更新
- MITRE ATT&CK、NIST CSF 2.0、AI 関連ガイドラインなどの参照情報と表示を更新
- 既存の診断結果は引き続き利用可能
...
- **定期的なセキュリティレビュー** — その時点の防御状況を整理し、着手候補を明確にする
- **経営層への報告** — セキュリティ状況・弱点・推奨対策を報告資料として整理する
- **対応候補の検討** — 時間軸別の整理を使って、どの対策から着手すべきかを判断する
- **ガイドライン参考確認** — IPA・経産省・NIST CSF 2.0 を参考に対応状況を確認する
...
### 攻撃シナリオ耐性評価 / 対応候補
![攻撃シナリオと対応候補](manual/images/ja/attack-scenarios.png)

### フレームワーク参考確認（NIST CSF 2.0）
![NIST CSF 参考確認](manual/images/ja/framework-csf.png)
